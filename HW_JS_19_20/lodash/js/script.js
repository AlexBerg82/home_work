...
var returnObj = JSON.stringify(data);
var seriaData = JSON.parse(returnObj);
...
_.each(seriaData, function(obj, key){
	_.each(obj, function(valObj, keyObj){
		if(keyObj == 'skills'){
			_.each(obj.skills, function(valSkil, keySkil){
				withSkill = valSkil.toLowerCase();
				arrSkill.push(withSkill);
				arrNm = _.union(arrSkill);
			});
		}
	});
});

document.write('<h1 style="color:blue;">Задание 1</h1>'+arrNm.sort().join('<br>'));



//Массив имен (поле name) людей, отсортированных в зависимости от количества их друзей (friends)
_.each(seriaData, function(objData, keyData){
	_.each(objData, function(valObjData, keyObjData){
	
		if(keyObjData == 'name'){
			name = valObjData;
		}
		if(keyObjData == 'friends'){
			leng = valObjData.length;
		}
...
document.write('<h1 style="color:blue;">Задание 2</h1>'+arr.sort().join('<br>'));



//Массив всех друзей всех пользователей, не должно быть повторяющихся людей
_.each(seriaData, function(obj2, key2){
	_.each(obj2, function(valObj2, keyObj2){

		if(keyObj2 == 'name'){
			user = valObj2;
				
			_.each(obj2.friends, function(val2, keys){
				_.each(val2, function(val2, keys2){
...
var arrSortFrnd = _.union(arrFrnd);
document.write('<h1 style="color:blue;">Задание 3</h1>'+arrSortFrnd.sort().join('<br>'));
